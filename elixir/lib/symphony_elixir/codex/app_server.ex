defmodule SymphonyElixir.Codex.AppServer do
  ...
  def start_session(workspace, opts \\ []) do
    ...
    issue_state = Keyword.get(opts, :state)
    command = Config.codex_command_for_state(issue_state)

    Logger.debug(
      "Codex command resolved state=#{inspect(issue_state)} default?=#{command == Config.settings!().codex.command}"
    )
